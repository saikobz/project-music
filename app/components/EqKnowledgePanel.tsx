...
const TAG_STYLE: Record<EqGuideLink["tag"], string> = {
  พื้นฐาน: "border-[#2A2A2A] bg-[#1A1A1A] text-[#8E8E8E]",
  มิกซ์: "border-[#2A2A2A] bg-[#1A1A1A] text-[#8E8E8E]",
  โทนเสียง: "border-[#E5A93D]/40 bg-[#E5A93D]/10 text-[#E5A93D]",
};

// ไอคอนตกแต่งเพื่อบอกว่าลิงก์นี้จะเปิดออกไปภายนอก
function ExternalIcon() {
  return (
    <svg className="h-4 w-4 text-[#E5A93D]" viewBox="0 0 20   20" fill="none" aria-hidden="true">
...
export default function EqKnowledgePanel() {
  ...
  return (
    <aside className="fixed left-3 z-40 bottom-3 md:bottom-auto md:top-1/2 md:-translate-y-1/2">
      <nav
        className={`overflow-hidden rounded-2xl border border-[#2A2A2A] bg-[#0A0A0A]/92 shadow-2xl backdrop-blur-lg transition-all duration-300 ${
      ...
      >
        <button
          ...
          className="cursor-pointer flex h-14 w-full items-center gap-2 px-3 text-[#F3F3F3] transition hover:bg-[#121212]"
        >
          <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-[#2A2A2A] bg-[#121212]">
            <svg className="h-4 w-4 text-[#E5A93D]" viewBox="0 0 20 20" fill="none" aria-hidden="true">
          ...
          >
            <span className="block text-xs font-semibold tracking-wide text-[#8E8E8E] cursor-pointer">EQ GUIDE</span>
              ...
              {EQ_GUIDE_LINKS.map((link) => (
                <li key={link.url}>
                  <a
                    ...
                    className="flex min-h-10 items-center justify-between gap-2 rounded-xl border border-[#2A2A2A] bg-[#121212] px-3 py-2 text-sm text-[#F3F3F3] transition hover:border-[#555555] hover:bg-[#1A1A1A] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E5A93D]/60"
                  >
