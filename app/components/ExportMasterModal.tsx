...
export default function ExportMasterModal({ isOpen, onClose, onExport, isExporting }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#050505]/80 backdrop-blur-md transition-opacity p-4">
      <div className="w-full max-w-[540px] rounded-2xl border border-[#2C2824] bg-[#0D0B0A] p-6 shadow-2xl">
        ...
        <div className="mb-6 grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 rounded-lg border border-[#2C2824] bg-[#161412] p-3">
              {STEM_OPTIONS.map((option) => (
                <label key={option.value} className="flex cursor-pointer items-center gap-3 text-sm text-gray-300 transition-colors hover:text-white">
                  <input
                    ...
                    className="h-4 w-4 rounded border-[#2C2824] accent-[#E5A93D]"
                  />
        ...
        {exportType === "mix" ? (
          ...
          <div className="mb-8">
            ...
            <div className="rounded-lg border border-[#2C2824] bg-[#161412]/50 p-4 text-center text-xs text-[#8E8E8E]">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 rounded-xl bg-[#E5A93D] py-3.5 text-sm font-bold uppercase tracking-wider text-[#0D0B0A] transition-all hover:bg-[#F3C05D] disabled:opacity-50 shadow-[0_0_15px_rgba(229,169,61,0.2)]"
          >
            {isExporting ? (
              <span className="flex items-center justify-center gap-2">
                <span className="h-4 w-4 animate-spin rounded-full border-2 border-[#0D0B0A] border-t-transparent"></span>
                Processing...
              </span>
            ) : (
              "Download"
